...
with warnings.catch_warnings(record=True) as w:
    ...
    print("✓ Fit succeeded with warning:")
    for warning in w:
        print(f"  {warning.category.__name__}: {warning.message}")
...
sl_cv = SuperLearner(learners=learners, method='nnloglik', cv=3, random_state=42)
...
|---------|--------|-------------|
| **Final refit error handling** | ✅ Complete | Catches all exceptions, adds dummy learners, issues warnings |
| **Prediction error tracking** | ✅ Complete | Tracks errors, uses neutral probability (0.5), warns user |
| **Min viable learners** | ✅ Complete | Configurable threshold for minimum successful learners |
| **Comprehensive diagnostics** | ✅ Complete | Detailed error records with phase, type, severity |
| **Failed learner tracking** | ✅ Complete | `failed_learners_` attribute tracks all failures |
| **Graceful degradation** | ✅ Complete | Dummy learners ensure ensemble continues working |
| **CV error tracking** | ✅ Complete | Errors during cross-validation are tracked |
| **Sample weight support** | ✅ Complete | Full support with TypeError handling |
